...
# 학습된 모델 파일 경로
TRAINED_MODEL = "/root/alexnet_cifar10.h5"

# Argument Parsing
def parse_args():
    parser = argparse.ArgumentParser(
        description="AlexNet inference with GPU memory limitation and CPU core limitation."
    )
    parser.add_argument(
...
# GPU Setup
def setup_gpu(use_gpu=True, memory_growth=False, gpu_mem_limit=512):
    if not use_gpu:
...
def load_and_prepare_data(batch_size=64):
